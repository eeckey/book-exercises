...
# longish). Make the sentence lowercase; you can use a function to help.
  sentence <- tolower("Happiness is when what you think, what you say, and what you do are in harmony")
...
# Extract the vector of letters from the resulting list
  vector_of_letters <- letter.list[[1]]
...
# Hint: use a filter operation!
  count_occurrences <- function(letter, letter_vector){
    occurances <- letter_vector[letter_vector == letter]
    return(length(occurances))
  }

# Call your `count_occurrences()` function to see how many times the letter 'e'
# is in your sentence.
  count_occurrences("e", vector_of_letters)
...
# Convert the result into a list (using `as.list()`).
  frequencies <- as.list(sapply(unique_letters, count_occurrences, vector_of_letters))
