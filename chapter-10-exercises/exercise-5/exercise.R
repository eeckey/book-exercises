...
popular_2013 <- max(names_2013$name)
...
most_popular_in_year <- function(year){
  most_popular_year <- names[names$year == year, ]
  most_popular_name <- max(most_popular_year$name)
  return(most_popular_name)
}

# What was the most popular female name in 1994?
most_popular <- most_popular_in_year(1994)
...
number_in_million <- function(name, year){
  year <- names[names$year == year, ]
  name <- year[year$name == name, ]
  name_count <- name$prop
  babies_in_million <- name_count/1000000
  return(babies_in_million)
}
...
Emily_2001 <- number_in_million('Emily', 2001)
